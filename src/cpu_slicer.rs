...
impl CPUSlicer {
            ...
            .filter_map(|plane_z| {
                ...
                for polygon in &polygons {
                    ...
                    let points: Vec<Point<i32>> = polygon
                        .iter()
                        .map(|p| {
                            let (x, y) = CPUSlicer::model_to_image_coords(
                                p,
                                min_x,
                                min_y,
                                scale,
    ...
    pub fn rotate_triangles(triangles: &[Triangle], rotation_matrix: &Matrix3<f64>) -> Vec<Triangle> {
        triangles
            .par_iter()
            .map(|tri| CPUSlicer::rotate_triangle(tri, rotation_matrix))
            .collect()
    }
    ...
    fn rotate_triangle(triangle: &Triangle, rotation_matrix: &Matrix3<f64>) -> Triangle {
        let vertices = [
            CPUSlicer::rotate_point(&triangle.vertices[0], rotation_matrix),
            CPUSlicer::rotate_point(&triangle.vertices[1], rotation_matrix),
            CPUSlicer::rotate_point(&triangle.vertices[2], rotation_matrix),
        ];
    ...
    fn compute_slice_area(triangles: &[Triangle], plane_z: f64) -> Vec<f64> {
        ...
        let slice_areas: Vec<f64> = polygons
            .iter()
            .map(|polygon| CPUSlicer::polygon_area(polygon))
            .collect();
    ...
    pub fn compute_cross_sections(
        ...
        let areas: Vec<f64> = slice_z_values
            .par_iter()
            .flat_map(|&plane_z| {
                CPUSlicer::compute_slice_area(triangles, plane_z)
            })
